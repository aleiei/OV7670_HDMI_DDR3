...
//	OV7670 : VGA RGB565
	SET_OV7670 + 0 	: 	LUT_DATA	= 	16'h1204;	// VGA RGB565
	...
	SET_OV7670 + 165 :	LUT_DATA	=	16'h4200;	// COM17: disable sensor DSP color bars

	// ------------------------------------------------------------------
	// Optional manual image-control overrides (all disabled by default).
	//
	// To enable this block, change LUT_SIZE from 168 to 188 in
	// ov7670_init.v. For every control you want to use, comment out its
	// 16'h4200 placeholder and uncomment exactly one replacement at the
	// same index. Unused slots safely repeat COM17=00 (color bars off).
	// Keep automatic controls enabled unless you specifically need manual
	// gain, exposure, or white balance. See OV7670_MANUAL_CONTROLS.md.
	// ------------------------------------------------------------------

	// Image orientation, MVFP (0x1E): normal=01, mirror=21, flip=11, both=31.
	// SET_OV7670 + 166 : LUT_DATA = 16'h1e01;
	SET_OV7670 + 166 : LUT_DATA = 16'h4200;

	// Brightness, BRIGHT (0x55): 00 neutral, 10 lighter, 20 more lighter,
	// 90 darker, A0 more darker. Bit 7 selects the negative direction.
	// SET_OV7670 + 167 : LUT_DATA = 16'h5500;
	SET_OV7670 + 167 : LUT_DATA = 16'h4200;

	// Contrast, CONTRAS (0x56): 40 nominal; lower/higher values reduce/increase it.
	// SET_OV7670 + 168 : LUT_DATA = 16'h5640;
	SET_OV7670 + 168 : LUT_DATA = 16'h4200;

	// COM8 automatic-control mask: FF all automatic, FD manual AWB,
	// FB manual gain, FE manual exposure, F8 all three manual.
	// SET_OV7670 + 169 : LUT_DATA = 16'h13ff;
	SET_OV7670 + 169 : LUT_DATA = 16'h4200;

	// Manual gain (0x00), used only when COM8 AGC is disabled: 00..FF.
	// SET_OV7670 + 170 : LUT_DATA = 16'h0040;
	SET_OV7670 + 170 : LUT_DATA = 16'h4200;

	// Manual exposure, used only when COM8 AEC is disabled. Exposure is
	// {AECHH[5:0], AECH[7:0], COM1[1:0]}; change the three values together.
	// SET_OV7670 + 171 : LUT_DATA = 16'h0400; // COM1: exposure bits 1:0
	SET_OV7670 + 171 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 172 : LUT_DATA = 16'h1040; // AECH: exposure bits 9:2
	SET_OV7670 + 172 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 173 : LUT_DATA = 16'h0700; // AECHH: exposure bits 15:10
	SET_OV7670 + 173 : LUT_DATA = 16'h4200;

	// Manual white balance, used only when COM8 AWB is disabled.
	// SET_OV7670 + 174 : LUT_DATA = 16'h0140; // BLUE gain: 00..FF
	SET_OV7670 + 174 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 175 : LUT_DATA = 16'h0240; // RED gain: 00..FF
	SET_OV7670 + 175 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 176 : LUT_DATA = 16'h6a40; // GREEN gain: 00..FF
	SET_OV7670 + 176 : LUT_DATA = 16'h4200;

	// Color matrix / saturation. Change MTX1..MTX6 and MTXS together;
	// these defaults reproduce the currently working color matrix.
	// SET_OV7670 + 177 : LUT_DATA = 16'h4f80; // MTX1
	SET_OV7670 + 177 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 178 : LUT_DATA = 16'h5080; // MTX2
	SET_OV7670 + 178 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 179 : LUT_DATA = 16'h5100; // MTX3
	SET_OV7670 + 179 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 180 : LUT_DATA = 16'h5222; // MTX4
	SET_OV7670 + 180 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 181 : LUT_DATA = 16'h535e; // MTX5
	SET_OV7670 + 181 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 182 : LUT_DATA = 16'h5480; // MTX6
	SET_OV7670 + 182 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 183 : LUT_DATA = 16'h589e; // MTXS sign bits
	SET_OV7670 + 183 : LUT_DATA = 16'h4200;

	// Edge enhancement. COM16 bit 5 enables it; EDGE sets strength.
	// SET_OV7670 + 184 : LUT_DATA = 16'h4138; // COM16: edge enhancement enabled
	SET_OV7670 + 184 : LUT_DATA = 16'h4200;
	// SET_OV7670 + 185 : LUT_DATA = 16'h3f00; // EDGE: 00 minimum, FF maximum
	SET_OV7670 + 185 : LUT_DATA = 16'h4200;
	default		 :	LUT_DATA	=	0;
	endcase
end
endmodule
